...
class DKN(object):
    ...
    def _build_inputs(self, args):
        with tf.name_scope('input'):
            ...
            self.news_entities = tf.placeholder(dtype=tf.int32, shape=[None, args.max_title_length],
                                                name='news_entities')
    ...
    def _build_model(self, args):
        with tf.name_scope('embedding'):
            ...
            # 对实体和上下文进行非线性变换
            if args.transform:
                self.entity_embeddings = tf.layers.dense(self.entity_embeddings, units=args.entity_dim,
