...
class Model(object):
    ...
    # def build_model(self, uij,recall_skn_idx):
    def build_model(self, uij):
        ...
        self.sl = uij[2]
        self.last = uij[3]
        # self.recall_skn_idx=recall_skn_idx
        # emb variable
        ...
        if self.is_training:
            ...
            # recall_brand=tf.gather(brand_list,self.recall_skn_idx)
            # recall_msort = tf.gather(msort_list, self.recall_skn_idx)
            #
            # recall_emb=tf.concat([tf.nn.embedding_lookup(item_emb_w, self.recall_skn_idx),
            #                      tf.nn.embedding_lookup(brand_emb_w, recall_brand),
