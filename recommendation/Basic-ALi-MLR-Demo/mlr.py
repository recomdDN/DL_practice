...
# tf.add_to_collection("losses", tf.contrib.layers.l2_regularizer(0.001)(u))
# tf.add_to_collection("losses", tf.contrib.layers.l1_regularizer(0.001)(u))
w = tf.Variable(tf.random_normal([108, m], 0.0, 0.5), name='w')
# tf.add_to_collection("losses", tf.contrib.layers.l2_regularizer(0.001)(w))
# tf.add_to_collection("losses", tf.contrib.layers.l1_regularizer(0.001)(w))
...
with tf.Session() as sess:
    ...
    for epoch in range(0, 2000):
        ...
        if epoch % 100 == 0:
            ...
            cost_, predict_test = sess.run([cost, pred], feed_dict=f_dict)
